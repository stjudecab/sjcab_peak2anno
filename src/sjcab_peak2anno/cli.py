...
def run(args: argparse.Namespace) -> Optional[Path]:
    ...
    if args.command in {"loop2gene", "loop2feature", "loop2state"}:
        ...
        return annotate_loop(
            ...
            tss_bed=args.tss_bed,
            gene_bed=args.gene_bed,
            gene_type=getattr(args, "gene_type", "nomicro"),
            feature_dir=args.feature_dir,
...
def database_install_command(args: argparse.Namespace) -> Optional[list[str]]:
    ...
    if needs_gene and not has_gene_override:
        if version in {"def", "default"}:
            command = ["sjcab-peak2anno-db", "install-genebed", str(species)]
            if data_dir:
                command.extend(["-d", data_dir])
        else:
            command = ["sjcab-peak2anno-db", "install-genebed", str(species), str(version)]
            if data_dir:
    ...
    if needs_feature and not has_feature_override:
        ...
        if version in {"def", "default"}:
            command = ["sjcab-peak2anno-db", "install-feature", str(species)]
            if data_dir:
                command.extend(["-d", data_dir])
        else:
            command = ["sjcab-peak2anno-db", "install-feature", str(species), str(version)]
            if data_dir:
                command.extend(["-o", data_dir])
        return command
    return None
...
def offer_database_install(args: argparse.Namespace, missing: Exception) -> bool:
    """Install missing database files, returning whether to retry."""
    ...
    if automatic:
        print("peak2anno: installing the selected database reference:", file=sys.stderr)
